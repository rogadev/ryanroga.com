...
async function refreshSnapshots(logger: AstroIntegrationLogger): Promise<void> {
	...
	for (const site of DEMO_SITES) {
		for (const page of site.pages) {
			...
			try {
				// eslint-disable-next-line no-await-in-loop -- capture pages one at a time to keep load on the live site gentle
...
async function replaceAsync(
	...
	for (const match of input.matchAll(pattern)) {
		// eslint-disable-next-line no-await-in-loop -- replacements must land in document order
