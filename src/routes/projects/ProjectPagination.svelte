<script lang="ts">
	let { currentPage = 0, numberOfPages = 0, onPageChange } = $props();
	...
	const previousPage = () => {
		...
		if (currentPage > 1) onPageChange?.({ currentPage: currentPage - 1 });
	};

	const nextPage = () => {
		if (currentPage === 0) return;
		if (currentPage < numberOfPages) onPageChange?.({ currentPage: currentPage + 1 });
	};
</script>
